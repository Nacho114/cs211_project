...
void draw() {
  ...
  PImage sobelImg = sobel(camImg);
  image(camImg/*sobelImg*/, 0, 0);
...
ArrayList<PVector> hough(PImage edgeImg) {
  ...
  for (int i = 0; i < 12; ++i) {
    if (bestCandidates.size()>i) {
      displayAccElem(bestCandidates.get(i), rDim, phiDim, discretizationStepsR, discretizationStepsPhi, edgeImg.width);
      ret.add(vectorForElem(bestCandidates.get(i), rDim, phiDim, discretizationStepsR, discretizationStepsPhi));
    }
  }
  
  getIntersections(ret);
  
  return ret;
}

PVector vectorForElem(int idx, int rDim, int phiDim, float discretizationStepsR, float discretizationStepsPhi) {
...
void displayAccElem(int idx, int rDim, int phiDim, float discretizationStepsR, float discretizationStepsPhi, int imgWidth) {
  // first, compute back the (r, phi) polar coordinates:
  int accPhi = (int) (idx / (rDim + 2)) - 1;
  int accR = idx - (accPhi + 1) * (rDim + 2) - 1;
  float r = (accR - (rDim - 1) * 0.5f) * discretizationStepsR;
